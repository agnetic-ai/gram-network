#!/usr/bin/env python3
"""Gram Network Auto-Claim — FAST version (no social tasks, <30s total)"""
...
for name, raw in ACCOUNTS:
    ...
    try:
        ...
        # Mining cycle only (no social tasks)
        ...
        report.append({'name': name, 'error': str(e)[:80]})
...
for r in report:
    if 'error' in r:
        lines.append(f"{r['name']:10s} ERROR: {r['error']}")
    else:
        acts = ','.join(r['actions']) if r['actions'] else 'idle'
        lines.append(f"{r['name']:10s}{r['balance']:>8.2f}  {acts}")
